...
class BasicBarChart: UIView {
    ...
    let barWidth: CGFloat = 50.0
    
    /// space between each bar
    let space: CGFloat = 35.0
    ...
    var dataEntries: [BarEntry]? = nil {
        didSet {
            ...
            if let dataEntries = dataEntries {
                scrollView.contentSize = CGSize(width: (barWidth + space)*CGFloat(dataEntries.count), height: self.frame.size.height-50)
    ...
    override func layoutSubviews() {
        scrollView.frame = CGRect(x: 0, y: (-self.frame.size.height/2)+20, width: self.frame.size.width, height: self.frame.size.height + self.frame.size.height/3)
    }
    ...
    private func showEntry(index: Int, entry: BarEntry) {
        ...
        let yPos: CGFloat = translateHeightValueToYPosition(value: entry.height/2)
        ...
        if entry.height > 0 {
            ...
            /// Draw text above the bar
            drawTextValue(xPos: xPos - space/2, yPos: yPos, textValue: entry.textValue, color: entry.color)
            
            /// Draw text below the bar
            drawTitle(xPos: xPos - space/2, yPos: mainLayer.frame.height - bottomSpace - 20, title: entry.title, color: entry.color)
        }
    }
